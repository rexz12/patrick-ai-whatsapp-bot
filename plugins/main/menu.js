import { getCaseCount, getCasesByCategory } from "../../case/patrick.js";
import {
...
import {
  formatUptime,
  getTimeGreeting,
} from "../../src/lib/patrick-formatter.js";
import {
  getCommandsByCategory,
  getCategories,
} from "../../src/lib/patrick-plugins.js";
import { getDatabase } from "../../src/lib/patrick-database.js";
import fs from "fs";
import { getAssetBuffer } from "../../src/lib/patrick-asset-manager.js";
...
async function formatTime(date) {
  const timeHelper = await import("../../src/lib/patrick-time.js");
...
async function handler(m, { sock, config: botConfig, db, uptime }) {
  ...
  try {
    if (botConfig.assets && botConfig.assets["patrick"]) {
      imageBuffer = getAssetBuffer("patrick");
    }
    if (botConfig.assets && botConfig.assets["patrick2"]) {
      thumbBuffer = getAssetBuffer("patrick2");
    }
  } catch (e) {
      ...
      case 1:
        if (imageBuffer) {
          await sock.sendMessage(m.chat, {
            image: getAssetBuffer("patrick") || {
              url: "https://gimita.id/patrick.png",
            },
            caption: ``,
...
`,
      ...
      case 2:
        ...
        const media = await prepareWAMessageMedia({
          image: getAssetBuffer("patrick") || getAssetBuffer("patrick")
                  ...
                  nativeFlowMessage: {
                    messageParamsJson: JSON.stringify({
                      ...
                      tap_target_configuration: {
                        ...
                        canonical_url: "https://patrick.site",
      ...
      case 3:
        const content = {
          buttonsMessage: {
            ...
            locationMessage: {
              jpegThumbnail: await sharp(getAssetBuffer("patrick")).resize(300, 170).toBuffer(),
              name: config.bot.name,
...
> 🍬 *Register*: ${user.isRegistered ? "Sudah" : "Belum"}`,
      ...
      case 4: {
        const ppss = getAssetBuffer("patrick2");
        ...
        const media4 = await prepareWAMessageMedia({
          video: getAssetBuffer("patrick-mp4"),
          gifPlayback: true
        }, { upload: sock.waUploadToServer });
...
Enjoy your use brother.`
    ...
    if (audioEnabled && botConfig.assets && botConfig.assets["patrick-mp3"]) {
      const audioUrl = getAssetBuffer("patrick-mp3");
      if (!audioUrl) return;
      try {
        switch (menuVariant) {
          case 1:
            try {
              const oggPath = await (async () => {
                ...
                const audioBuf = getAssetBuffer("patrick-mp3");
          ...
          case 3: {
            ...
            await sock.sendMessage(m.chat, {
              audio: getAssetBuffer("patrick-mp3"),
              mimetype: "audio/mpeg",
              ptt: false,
            }, { quoted: qtext });
            break;
