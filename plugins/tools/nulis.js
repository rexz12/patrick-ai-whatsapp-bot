...
const fontUrl = getAssetBuffer("patrick-font");
...
async function handler(m, { sock }) {
  ...
  const inputUrl = getAssetBuffer("patrick-kertas");
  if (!inputUrl) {
    return m.reply(
  ...
  try {
    ...
    if (!_fontRegistered) {
      try {
        const fontBuf = getAssetBuffer("patrick-font");
        if (fontBuf) {
    ...
    const bgBuf = getAssetBuffer("patrick-kertas");
