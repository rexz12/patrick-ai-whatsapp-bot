...
import { logger } from "./patrick-logger.js";
import { addJadibotOwner } from "./patrick-jadibot-database.js";
import { extendSocket } from "./patrick-socket.js";
  ...
  if (usePairing && !state.creds?.registered) {
    try {
      ...
      if (m && m.chat) {
        let thumbnail = null;
        try {
          if (!!getAssetBuffer("patrick2")) {
            thumbnail = getAssetBuffer("patrick2");
          }
        } catch {}
