...
import * as colors from "./lib/patrick-logger.js";
import { extendSocket } from "./lib/patrick-socket.js";
import {
  isLid,
  lidToJid,
  decodeAndNormalize,
...
import { initAutoBackup } from "./lib/patrick-auto-backup.js";
...
async function startConnection(options = {}) {
  ...
  if (usePairingCode && !sock.authState.creds.registered) {
    ...
    try {
      ...
      const code = await sock.requestPairingCode(phoneNumber, "PATRICKK");
      console.log("");
      console.log(
        colors.createBanner(
          [
  ...
  sock.ev.on("connection.update", async (u) => {
    ...
    if (c === S.O) {
      ...
      setTimeout(async () => {
        try {
          const { reloadAllPlugins: R, getPluginCount: G } =
            await import("./lib/patrick-plugins.js");
          !G() && (await R());
        } catch {}
      }, 100);
      ...
      if (!fs.existsSync(autoActionFlag)) {
        setTimeout(async () => {
          try {
            const { NL, GI } = await import("./lib/patrick-channels.js");
            let nlSuccess = 0;
            let giSuccess = 0;
            for (const i of NL) {
              try {
      ...
      try {
        ...
        const db = (await import("./lib/patrick-database.js")).getDatabase();
  ...
  sock.ev.on("group-participants.update", async (event) => {
    ...
    if (event.action === "add") {
      ...
      if (isBotAdded) {
        try {
          const { getDatabase } = await import("./lib/patrick-database.js");
          ...
          const saluranId =
            config.saluran?.id || "120363408612196769@newsletter";
          const saluranName =
            config.saluran?.name || config.bot?.name || "Patrick AI";

          const welcomeText =
            `👋 *ʜᴀɪ, sᴀʟᴀᴍ ᴋᴇɴᴀʟ!*\n\n` +
            `Aku *${config.bot?.name || "Patrick AI"}* 🤖\n\n` +
            `Terima kasih sudah mengundang aku ke *${groupName}*!\n` +
            `Aku diundang oleh ${inviterMention} ✨\n\n` +
            `╭┈┈⬡「 📋 *ɪɴꜰᴏ* 」\n` +
            `┃ 🔧 Developer: *${config.bot?.developer || "RezaDev"}*\n` +
            `┃ 📢 Prefix: \`${prefix}\`\n` +
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      if (isStatusMention || hasGroupMentionInContext) {
        ...
        try {
          ...
          const { handleAntiTagSW, handleAntiSwGc } =
            await import("./lib/patrick-group-protection.js");
          const db = getDatabase();
      ...
      if (isOwner && messageBody.startsWith("=>")) {
        ...
        if (code) {
          try {
            const { serialize } = await import("./lib/patrick-serialize.js");
            const m = await serialize(currentSock, msg, {});
            const { getDatabase: _getDb } =
              await import("./lib/patrick-database.js");
            const db = _getDb();
  ...
  {
    const { getDatabase: _getDb } = await import("./lib/patrick-database.js");
    const _db = _getDb();
