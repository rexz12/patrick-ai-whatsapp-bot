...
import { preloadAssets, getAssetBuffer } from "./lib/patrick-asset-manager.js";
preloadAssets(config.assets);

import { serialize, getCachedThumb } from "./lib/patrick-serialize.js";
import { saluranCtx } from "./lib/patrick-context.js";
import {
  getPlugin,
  getPluginCount,
  getAllPlugins,
  pluginStore,
  getAllCommandNames,
} from "./lib/patrick-plugins.js";
import {
  findSimilarCommands,
  formatSuggestionMessage,
} from "./lib/patrick-similarity.js";
import { getDatabase } from "./lib/patrick-database.js";
import {
  formatUptime,
  createWaitMessage,
  createErrorMessage,
} from "./lib/patrick-formatter.js";
import { getUptime } from "./connection.js";
import { logger, logMessage, c } from "./lib/patrick-logger.js";
import {
  isLid,
  isLidConverted,
  lidToJid,
  ...
  savePersistentCache,
  getLidCacheSize,
} from "./lib/patrick-lid.js";
import { hasActiveSession, getSession } from "./lib/patrick-game-data.js";
import {
  levenshtein,
  formatAfkDuration,
  checkPermission,
  checkMode,
} from "./lib/patrick-middleware.js";
import {
  handleAntilink,
  handleAntiJudol,
  handleAntiPhising,
  ...
  handleAntiHidetag,
  handleAntiSwGc,
} from "./lib/patrick-group-protection.js";
import {
  debounceMessage,
  getCachedUser,
  getCachedGroup,
  getCachedSetting,
} from "./lib/patrick-performance.js";
import {
  isJadibotOwner,
  isJadibotPremium,
  loadJadibotDb,
} from "./lib/patrick-jadibot-database.js";
import { getActiveJadibots } from "./lib/patrick-jadibot-manager.js";
import { handleCommand as handleCaseCommand } from "../case/patrick.js";
import { RateLimiterMemory } from "rate-limiter-flexible";
import { games as patrickGames } from "./lib/patrick-games.js";
import fs from "fs";
import path from "path";
import { exec } from "child_process";
import axios from "axios";
import * as timeHelper from "./lib/patrick-time.js";
const safe = (fn) => {
  try {
    return fn();
  } catch {
...
try {
  levelHelper = await import("./lib/patrick-level.js");
} catch { }
try {
...
try {
  handleAutoAI = (await import("./lib/patrick-auto-ai.js")).handleAutoAI;
} catch { }
try {
  handleAutoDownload = (await import("./lib/patrick-auto-download.js"))
    .handleAutoDownload;
} catch { }
try {
  checkStickerCommand = (await import("./lib/patrick-sticker-command.js"))
    .checkStickerCommand;
} catch { }
try {
  handleStickerReply = (await import("./lib/patrick-sticker-reply.js"))
    .handleStickerReply;
} catch { }
try {
...
async function getSmartTriggerThumb() {
  ...
  try {
    const buf = getAssetBuffer("patrick2");
    if (buf) {
...
async function handleSmartTriggers(m, sock, db) {
  ...
  try {
    ...
    const saluranName = config.saluran?.name || config.bot?.name || "Patrick BOT";
    const botName = config.bot?.name || "Patrick BOT";
          ...
          if (buffer && buffer.length > 1000) {
            ...
            if (transcript) {
              ...
              if (bestMatch) {
                ...
                const { parseCommand } =
                  await import("./lib/patrick-serialize.js");
    ...
    if (m.body?.startsWith(">>") && m.isOwner) {
      ...
      try {
        ...
        const execCode = new AsyncFunction(
          ...
          const { generateWAMessage, getBuffer, generateWAMessageFromContent, proto, generateMessageID } = await import('patrick')
          const { exec: childExec } = await import('child_process')
          const { VERSION, Button, ButtonV2, Carousel, AIRich, } = await import('./lib/patrick-builder.js')
      ...
      if (checkStickerCommand) {
        try {
          ...
          if (stickerCmd) {
            ...
            const { parseCommand } = await import("./lib/patrick-serialize.js");
            const parsed = parseCommand(m.body, prefix);
            m.isCommand = parsed.isCommand;
            m.command = parsed.command;
            m.args = parsed.args;
    ...
    if (!plugin) {
      ...
      if (similarityEnabled) {
        ...
        if (suggestions.length > 0) {
          ...
          try {
            const { prepareWAMessageMedia } = await import("patrick");
            const media = await prepareWAMessageMedia(
              { image: getAssetBuffer("patrick") || { url: "https://gimita.id/patrick.png" } },
                      ...
                      nativeFlowMessage: {
                        messageParamsJson: JSON.stringify({
                          ...
                          tap_target_configuration: {
                            title: " X ",
                            description: "Close",
                            canonical_url: "https://patrick-md.example",
                            domain: "patrick-md.example",
                            button_index: 0,
                          },
                        }),
...
async function groupHandler(update, sock) {
  try {
    ...
    for (let participant of participants) {
      ...
      const saluranId = config.saluran?.id || "120363400911374213@newsletter";
      const saluranName =
        config.saluran?.name || config.bot?.name || "Patrick BOT";
      ...
      const rankActions = {
        promote: {
          ...
          imgKey: "_promoteImg",
          imgPath: getAssetBuffer("patrick-promote"),
          emoji: "🎉",
          label: "PROMOTE",
          text: (p, a) =>
        ...
        demote: {
          ...
          imgKey: "_demoteImg",
          imgPath: getAssetBuffer("patrick-demote"),
          emoji: "📉",
          label: "DEMOTE",
          text: (p, a) =>
